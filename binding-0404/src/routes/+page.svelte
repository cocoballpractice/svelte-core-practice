...
<!-- let fruits = ['Apple', 'Banana', 'Cherry'] 선택지-->
<!-- let selectedFruits 담을것-->
<section>
	<h2>Checkbox 다중 선택</h2>
	<strong>Selected: {selectedFruits}</strong>
	{#each fruits as fruit}
		<label>
			<input type="checkbox" value={fruit} bind:group={selectedFruits} /> <!--fruit는 단방향, group 중요-->
			{fruit}
		</label>
	{/each}
</section>
...
<section>
	...
	<div>
		<select bind:value={select}>
			<option disabled value="">하나를 꼭 고르삼</option>
			<option>Apple</option>
